...
<c:forEach var="dept" items="${model}">
	<tr>
		...
		<td>${dept.dname}</td> 
		<td> ${dept.loc}</td>
		<td><a href="<c:url value="/emplotee?cmd=deptupdate&deptno=${dept.deptno}"/>" class="btn">U</a></td>
		<td><a href="<c:url value="/emplotee?cmd=deptdelete&deptno=${dept.deptno}"/>" class="btn">D</a></td>
	</tr>
</c:forEach>
